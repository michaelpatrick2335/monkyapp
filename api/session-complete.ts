...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const sRes = await pool.query("INSERT INTO meditation_sessions (user_id,level,tier,duration_seconds,completed_at,bananas_earned) VALUES ($1,$2,$3,$4,$5,1) RETURNING *", [user.id, user.level, user.tier, durationSeconds, new Date().toISOString()]);
    ...
    const updatedRow = await updateUser(pool, user.id, { level: newLevel, bananas: newBananas, totalSessions: user.totalSessions+1, totalSecondsMediated: user.totalSecondsMediated+durationSeconds, streakDays: newStreak, lastSessionDate: today, tier: newTier, freeSessionsUsed: newFreeUsed });
