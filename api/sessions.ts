...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const r = await pool.query("SELECT * FROM meditation_sessions WHERE user_id = $1 ORDER BY completed_at DESC", [userRow.id]);
    return res.json(r.rows.map((row: any) => ({ id: row.id, userId: row.user_id, level: row.level, tier: row.tier, durationSeconds: row.duration_seconds, completedAt: row.completed_at, bananasEarned: row.bananas_earned })));
